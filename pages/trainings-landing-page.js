...
const CourseLandingPage = () => {
  ...
  //console.log("getAllRecordsState", getAllRecordsState?.length);
  ...
  useLayoutEffect(() => {
    if (getCookies("trainingFilterCategory")) {
      setLocationState(getCookies("trainingFilterCategory"));
    }
  }, [getCookies("trainingFilterCategory")]);
  ...
  const getAllRecordFunc1 = async (data, sea) => {
    //console.log("dataapiKey", data);
    setLoading(true);
    try {
      // if (token && menuId) {
      // //console.log('menuId', menuId, token)
  ...
  const GetStaticPages = async () => {
    setHeroLoading(true);
    try {
      const response = await endpoints.GetStaticPages("trainings-landing-page");
            ...
            <Container>
              <BreadcrumbDiv>
                <Breadcrumb>
                  ...
                  <Breadcrumb.Item>Trainings</Breadcrumb.Item>
                </Breadcrumb>
              </BreadcrumbDiv>
            </Container>
          </MainBreadcrumbDiv>
            ...
            <Head1Section>
              <Row>
                ...
                <Head1SectionDiv>
                  {totalRecord && (
                    <p>
                      Showing 1–{totalRecord} of {totalRecord} trainings
                    </p>
                  )}
                  {/* <Select
                          ...
                          <Container style={{ position: "relative" }}>
                            <Row style={{ margin: "0px 15px" }}>
                              ...
                              >
                                {data?.content?.tabs.map((item, index) => (
                                  <SwiperSlide>
                                    <SectionAfterHeroCol key={index} span={24}>
                                      <Row style={{flexWrap: "nowrap"}}>
                                        <ImgDiv>
                                        {(item?.image !== "" && item?.image !== null) && (
                                          <img
