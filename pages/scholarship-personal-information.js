...
import LoaderModal from "../src/components/Modal/LoaderModal";
...
const PersonalInformation = () => {
  ...
  const firstNameCookies = getCookies("firstName_EN");
  const lastNameCookies = getCookies("lastName_EN");
  ...
  useLayoutEffect(() => {
    removeCookies("scholarshipTrackingId");
  }, []);
  ...
  //console.log("openErrorModal", openErrorModal, emailOrPhoneErrorStatusState);
  ...
  const createLMSUsersFunc = async (data) => {
    ...
    try {
      ...
      //console.log("dataPayloadResponse", response?.data?.statusCode);

      if (response) {
        if (response?.data?.statusCode === "200") {
          router.push("/scholarship-authentication-otp");
        } else {
        ...
        <div onClick={() => setDropdownCaretState(false)}>
              ...
              <StyledSigninDiv>
                <StyledRegisterationRow gutter={[32, 32]}>
                  <Col xl={12} lg={12} md={12} xs={24} sm={24}>
                    ...
                    <BeforeFormP>
                      First things first, we would like to begin with your
                      contact details to register your application with us. We
                      would need this information to inform you about your
                      application status.
                    </BeforeFormP>
                    ...
                    <Row gutter={[16, 16]}>
                      <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                        ...
                        {firstNameState && (
                          <>
                            {signInState?.firstName_EN?.trim() === ""||signInState?.firstName_EN?.trim() == undefined && (
                              <StyledErrorP>First Name is Mandatory</StyledErrorP>
                            )}
                          </>
                        )}
                      </Col>
                      <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                        ...
                        {lastNameState && (
                          <>
                            {signInState?.lastName_EN?.trim() === ""||signInState?.lastName_EN?.trim() == undefined && (
                              <StyledErrorP>Last Name is Mandatory</StyledErrorP>
                            )}
                          </>
                        )}
                      </Col>
                          ...
                          <>
                            ...
                            signInState?.confirm_email?.trim() === "" ? (
                              ...
                              <>
                                {signInState?.email?.trim() !==
                                  signInState?.confirm_email?.trim() && (
                                  <StyledErrorP>Email not matched</StyledErrorP>
                                )}
                              </>
                            )}
                          </>
                    ...
                    <AfterFormPDiv>
                      <AfterFormP>
                        Kindly provide contact details (Mobile Number / Email
                        ID) of the account holder. R2 reserves the right to
                        reject the application in case third party contact
                        details are provided
                      </AfterFormP>
                    </AfterFormPDiv>

                    <NoteP>
                      For international numbers, please make sure you are
                      contactable on the number you have provided
                    </NoteP>
                    ...
                    <StyledEmailBySmsRow>
                        ...
                        {phoneNumberState && signInState?.phone_number <= 0 ? (
                          <>
                            ...
                            {signInState?.phone_number?.slice(0, 1) === "0" ? (
                              <StyledErrorP1>Invalid Number</StyledErrorP1>
                            ) : (
                              <StyledErrorP1>
                      ...
                      >
                        <StyledColoredPFlex>
                          ...
                          <StyledColoredP onClick={() => router.push("/terms")}>
                            terms & conditions.
                          </StyledColoredP>
                        </StyledColoredPFlex>
                      </Checkbox>
          ...
          <SignInRow>
            <Container>
              <FlexEndDiv>
                {ukMated !== true ||
                signInState?.firstName_EN?.trim() === "" ||
                signInState?.lastName_EN?.trim() === "" ||
                signInState?.email?.trim() === "" ||
                signInState?.confirm_email?.trim() === "" ||
                ...
                emailValidationState === "Invalid Email" ||
                signInState?.phone_number <= 0||signInState?.phone_number?.slice(0, 1) === "0" ? (
                  <>
                    <CustomButton
                      customStyle={{
                        paddingInline: 30,
