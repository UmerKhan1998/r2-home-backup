...
const dashboard = () => {
  ...
  const [selectItem, setSelectItem] = useState("/ar/activities-surveys");
  ...
  const dashboardGetAssessmentFunc = async () => {
    try {
      const response = await endpoints.dashboardGetSurvey(
        authToken,
        surveysTable
      );
  ...
  const GetInstructorFunc = async () => {
    try {
      const response = await endpoints.GetInstructor(authToken);
      if (response?.data?.statusCode === "200") {
