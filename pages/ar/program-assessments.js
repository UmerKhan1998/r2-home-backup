...
const dashboard = () => {
  ...
  const [selectItem, setSelectItem] = useState("/ar/program-assessments");
  ...
  const GetInstructorFunc = async () => {
    try {
      const response = await endpoints.GetInstructor(authToken, "Program");
      if (response?.data?.statusCode === "200") {
