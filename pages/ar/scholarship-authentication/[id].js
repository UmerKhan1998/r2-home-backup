...
const PersonalInformation = () => {
  ...
  const createLMSAuthorizationFunc = async (code, otp) => {
    try {
      ...
      if (code || otp) {
        const response = await endpoints.GetTrackApplicationScholarship(
          code,
          otp
        );
        //console.log("responseShantosimple", response?.data?.statusCode);
        if (response?.data.statusCode !== "200") {
          toast.error("Invalid OTP");
        }

        setCookies("trackingCode", response?.data.data.registrationCode);
        setCookies(
          "scholarshipTrackingId",
          response?.data?.data?.registrationCode
        );
        ...
        if (response?.data.statusCode == "200") {
          ...
          if (response?.data?.data?.applicationStatus) {
            router.push("/ar/scholarship-track-your-application");
          } else {
  ...
  const getApplicationOTPFunc = async (code) => {
    try {
      ...
      if (code) {
        const response = await endpoints.getApplicationScholarshipOTP(code);
        if (response?.data?.message !== "Invalid OTP") {
          // router.push("/registeration-step");
          // toast.success("New Level Added Successfully");
          // router.push(`/authentication/${code}`);

          // console.log("responseData", response?.data?.data?.checklistId);
          setCookies("scholarshipId", response?.data?.data?.checklistId);
  ...
  useLayoutEffect(() => {
    //console.log("pathAauk", router.asPath?.split("/")[3]);
    getApplicationOTPFunc(router.asPath?.split("/")[3]);
  }, []);
  ...
  const GetValidateApplicationFunc = async (code) => {
    try {
      ...
      if (code) {
        const response = await endpoints.GetValidateApplicationScholarship(
          code
        );
        if (response) {
          //console.log("responseAuth", response?.data?.message);
          // if(response?.data?.message!=="Invalid Tracking Number"){
          //   router.push(`/scholarship-authentication/${code}`);
          // }else{
          //   toast.error('Invalid Tracking Number')
          // }
        }
