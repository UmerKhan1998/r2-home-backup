...
const PersonalInformation = () => {
  ...
  const handleInputChange = (e) => {
    ...
    //   //console.log(
    //     "router.asPath?.slice(16, 33)",
    //     router.asPath?.slice(19, 100)
    //   );
    //   createLMSAuthorizationFunc(router.asPath?.slice(19, 100), e);
    // }
  };
  ...
  const createLMSAuthorizationFunc = async (code, otp) => {
    try {
      ...
      if (code || otp) {
        const response = await endpoints.getTrackApplication(code, otp);
        //console.log("responseShantosimple", response?.data);
        setCookies("trackingCode", response?.data.data.registrationCode);
        setCookies("trackingId", response?.data?.data?.registrationCode);
        //console.log("responseShanto2", response?.data.data.courseTrainingId);
        setCookies("token1", response?.data?.message);
        ...
        //console.log("responseStatusCode", response?.data?.message);

        if (response?.data.statusCode == "200") {
          // if (response?.data?.data?.status) {
          if (
            response?.data?.data?.enrollmentStatus ||
            response?.data?.data?.applicationStatus
          ) {
  ...
  useLayoutEffect(() => {
    //console.log("pathAauk", router.asPath?.slice(19, 100));
    getApplicationOTPFunc(router.asPath?.slice(19, 100));
  }, []);
