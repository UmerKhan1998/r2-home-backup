...
const dashboard = () => {
  ...
  const [selectItem, setSelectItem] = useState("/ar/training-assessments");
  ...
  const GetInstructorFunc = async () => {
    try {
      const response = await endpoints.GetInstructor(authToken, "Training");
      if (response?.data?.statusCode === "200") {
