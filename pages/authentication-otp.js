...
const PersonalInformation = () => {
  ...
  const handleInputChange = (e) => {
    setSecurityCode(e);
    let fake = e;
    // //console.log("createLMSAuthorizationFunc");
  ...
  const createLMSAuthorizationFunc = async (data, code, wanIp) => {
    try {
      ...
      if (securityCode) {
        ...
        if (response?.data?.statusCode === "200") {
          // if (statusEnrollment === null || statusEnrollment === undefined || statusEnrollment === "") {
            ...
            const statusEnrollment=response?.data?.data?.enrollmentStatus
            setCookies("trackingId", response?.data?.data?.courseTrainingRegistrationsCode);
            // console.log("statusEnrollment", response?.data?.data?.courseTrainingRegistrationsCode);
  
            if (statusEnrollment === true) {
