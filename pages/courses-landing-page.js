...
import { getCookies, removeCookies } from "../src/helpers/cookie";
//swiper imports
...
import { CardImg1, R2Favicon } from "../images";
...
export async function getStaticProps() {
  const response = await endpoints.GetStaticPages("course-landing-page");
  const data = response?.data?.data;
  
  const getCourseTrainingMasterCategoryResponse = await endpoints.getCourseTrainingMasterCategory();
  const getCourseTrainingMasterCategoryResponseData = getCourseTrainingMasterCategoryResponse?.data?.data;

  const getCourseTrainingCategoryResponse = await endpoints.getCourseTrainingCategory();
  const getCourseTrainingCategoryResponseData = getCourseTrainingCategoryResponse?.data?.data;
  
  const getLevelResponse = await endpoints.getLevel();
  const getLevelResponseData = getLevelResponse?.data?.data;  

  const getLocationResponse = await endpoints.getLocation();
  const getLocationResponseData = getLocationResponse?.data?.data;  

  return {
    props: {
      initialProps: data || null,
      getCourseTrainingMasterCategoryResponseData: getCourseTrainingMasterCategoryResponseData || null,
      getCourseTrainingCategoryResponseData: getCourseTrainingCategoryResponseData || null,
      getLevelResponseData: getLevelResponseData || null,
      getLocationResponseData: getLocationResponseData || null,
    },
    revalidate: 5, // Set the revalidation interval in seconds
  };
}

const CourseLandingPage = ({ initialProps, getCourseTrainingMasterCategoryResponseData, getCourseTrainingCategoryResponseData, getLevelResponseData, getLocationResponseData }) => {
  ...
  useLayoutEffect(() => {
    if (getCookies("coursesFilterCategory")) {
      setLocationState(getCookies("coursesFilterCategory"));
    }
  }, [getCookies("coursesFilterCategory")]);
  ...
  const [filterArr, setFilterArr] = useState(getCourseTrainingMasterCategoryResponseData);
  ...
  //     // if (token && menuId) {
  //     // //console.log('menuId', menuId, token)
  //     const response = await endpoints.getCourseTrainingMasterCategory();
  //     if (response) {
  ...
  const [filterArr1, setFilterArr1] = useState(getCourseTrainingCategoryResponseData);

  const [filterArr2, setFilterArr2] = useState(getLevelResponseData);

  const [filterArr3, setFilterArr3] = useState(getLocationResponseData);
            ...
            <Head1Section>
              <Row>
                ...
                <Head1SectionDiv>
                  {totalRecord && (
                    <p>
                      Showing 1–{totalRecord} of {totalRecord} courses
                    </p>
                  )}
                  {/* <Select
      ...
      <body>
        ...
        <div>
            <>
              {GetStaticPagesState?.contentValue &&
                Object.values(
                  JSON.parse(GetStaticPagesState?.contentValue)
                )?.map((data,index) => (
                  <>
                    {data["type"] === "top_banner" && (
                      <>
  ...
  useLayoutEffect(() => {
    ...
    return () => window.removeEventListener("resize", updateMedia, {passive: true});
  }, []);
