import { Col, Collapse, Input, Modal, Rate, Row, Tabs } from 'antd';
...
import {
	...
	Show_More,
	Show_Less,
	Get_R2_Certificate_by_completing_the_entire_course,
	Download_Certificate,
	Request_for_Certificate,
	Your_Request_has_been_submitted_for_Certificate,
	Edit_your_Review,
	Edit_Review,
	Add_Review,
	No_Discussion_Found,
	You_have_already_applied_for_the_certificate,
	Your_zoom_class_will_start_at,
	Ok_text,
...
import { DownloadOutlined, DownOutlined, FieldTimeOutlined, LinkOutlined } from "@ant-design/icons";
...
const Inside = ({
	...
	const AddCourseTrainingReviewFunc = async () => {
		...
		try {
			if (addReviewParamsState?.review_EN === "") {
				...
				const response = await endpoints.CourseTrainingReview(authToken, addReviewParamsState);
	...
	console.log('editReviewLoading',editReviewLoading)

	const EditCourseTrainingReviewFunc = async () => {
		setEditReviewLoading(true)
		try {
            if (editReviewParamsState?.review_EN === "") {
                setIsRatingEmpty(true)
            } else {
	...
	const getCertificateRequestFunc = async () => {
		try {
			...
			const response = await endpoints.GetCertificateRequest(authToken, courseTrainingRegistrationId);
			if (response.data.statusCode === "200") {
	...
	const GetCertificateIssueByIdFunc = async () => {
		try {
			setCertificateRequestLoading(true)
			const response = await endpoints.GetCertificateCreate(authToken, courseTrainingRegistrationId);
			if (response?.data?.statusCode === "200") {
	...
	const GetStream = async (courseTrainingRegistrationId, courseTrainingFileId) => {
		window.scrollTo(0,0)
        try {
            const response = await endpoints.GetStream(authToken, courseTrainingRegistrationId, courseTrainingFileId);
            if (response?.data?.statusCode === "200") {
                setCourseVideoState(`${process.env.NEXT_PUBLIC_API_KEY}/api/mediastream/v1/MediaStreamServices/GetStreamById?Id=${response?.data?.data?.id}&Token=bearer ${authToken}`)
                setCustomVideoDemoPlayerState(false)
                setStreamId(response?.data?.data?.id);
                if(courseVideo !== null) {
                    courseVideo.current.currentTime = response?.data?.data?.setTime
                }
                playCourseVideo();
            }
        } catch (error) {
	...
	const GetStreamCourseTrainingDemoVideo = async (courseTrainingId, Language) => {
        try {
            const response = await endpoints.GetStreamCourseTrainingDemoVideo(authToken, courseTrainingId, Language);
            if (response?.data?.statusCode === "200") {
                setCourseVideoState(`${process.env.NEXT_PUBLIC_API_KEY}/api/mediastream/v1/MediaStreamServices/GetStreamCourseTrainingDemoVideoById?Id=${response?.data?.message}`)
            }
        } catch (error) {
            console.log("error", error);
        }
    ...
    useLayoutEffect(()=>{
        if(courseDetailState?.courseTrainingId) {
            GetStreamCourseTrainingDemoVideo(courseDetailState?.courseTrainingId, "Arabic");
        }
    }, [courseDetailState?.courseTrainingId])

	const MarkAttendanceLMSUserFunc = async (courseTrainingLinkVenueId) => {
		try {
			const response = await endpoints.MarkAttendanceLMSUser(authToken, courseTrainingLinkVenueId, courseTrainingRegistrationId);
			if (response?.data?.statusCode === "200") {
                getDashboardInside(courseId.id);
            }
		} catch (error) {
	...
	const [CustomVideoDemoPlayerState, setCustomVideoDemoPlayerState] = useState(true);
    const [CustomVideoPlayerBoolState, setCustomVideoPlayerBoolState] = useState(false);
    const CustomVideoPlayerFunc = () => {
        if (courseVideo.current.paused == false) {
            setCustomVideoPlayerBoolState(true);
        } else {
            setCustomVideoPlayerBoolState(false);
        }
    }

    const SetStreamTime = async (Id, TimeSecond) => {
        try {
            const response = await endpoints.SetStreamTime(authToken, Id, TimeSecond);
        } catch (error) {
            console.log("error", error);
        }
    }
    ...
    let intervalId;
    useLayoutEffect(()=>{
        if(CustomVideoPlayerBoolState) {
            intervalId = setInterval(() => {
                SetStreamTime(StreamId, courseVideo?.current?.currentTime)
    ...
    }, [CustomVideoPlayerBoolState])

	const getMinute = (items) => {
		var totalMinutes = 0;
		items.map((item, index) => (
			totalMinutes += parseInt(item?.duration_EN)
		));
	...
	const [certificateRequest, setCertificateRequest] = useState(true);
	...
	const RatingByCategory = (Category)=> {
		if (Category === "Course") {
			...
			return Program_Rating
		}
		else if (Category === "Webinar") {
			return Webinar_Rating
		}
		else if (Category === "Conference") {
			return Conference_Rating
		}
		else if (Category === "Symposium") {
			return Symposiums_Rating
		}
		else if (Category === "Workshop") {
			return Workshop_Rating
		}
	}

	const CertificateByCategory = (Category)=> {
		if (Category === "Course") {
			...
			return Program_Certificate
		}
		else if (Category === "Webinar") {
			return Webinar_Certificate
		}
		else if (Category === "Conference") {
			return Conference_Certificate
		}
		else if (Category === "Symposium") {
			return Symposiums_Certificate
		}
		else if (Category === "Workshop") {
			return Workshop_Certificate
		}
	}

	const SlugByCategory = (Category)=> {
		if (Category === "Course") {
			return 'course'
		}
		else if (Category === "Training") {
	...
	const checkActivetab = () => {
		if (getCookies("activeQa") === "false" && getCookies("activeReview") === "false" && getCookies("activeDiscussionBoard") === "false" && getCookies("activeCertificate") === "false") {
			return 0
		} else {
	...
	const [TabsIndex, setTabsIndex] = useState(checkActivetab());
    const TabsArrayKey = [
        Overview, 
        Q_A, 
        Reviews, 
    ...
    const TabsArray = [
		<CourseOverview>
			{courseDetailState?.overView_AR === undefined ? (
				<>
				...
				parse(`${courseDetailState?.overView_AR}`)
			)}
		</CourseOverview>,
		<CourseQa>
			<StyledCollapse
			...
			>
				{courseFaqState?.map((item, index) => (
					<Panel header={item?.question_AR} key={index}>
						<p>{item?.answer_AR}</p>
					</Panel>
				))}
				{(courseFaqState === undefined) && (
					<div style={{textAlign: "center", paddingBlock: "15px"}}>
						<img loading="lazy"src={Rolling.src} width="30px" height="30px" />
					</div>
				)}
			</StyledCollapse>
		</CourseQa>,
		<CourseReview>
			<h3>{Student_Feedback}</h3>
			<Row style={{ alignItems: "center" }}>
				<Col span={5}>
					<div className="course-rating">
						...
						<div className="rating-txt">{RatingByCategory(Category)}</div>
					</div>
				</Col>
				<Col span={19}>
			...
			<ReviewComments>
				{courseReviews?.dashboardInsideReviewViewModels.slice(0, ReviewCommentCount).map(
					(item, index) => (
						<>
							<div className="review-card" key={index}>
								...
								<div className="review-card-text">
									<h4>
										{item?.userName_AR}
										<span>{moment.utc(item?.insertDate).fromNow()}</span>
									</h4>
				...
				{(courseReviews?.totalRecord === 0) ? (
					...
					<>
						{(ReviewCommentCount < courseReviews?.totalRecord && courseReviews?.totalRecord !== 3) && (
							<SeeMoreBtn onClick={ReviewSeeMoreFunc}> {Show_More} </SeeMoreBtn>
						)}
				...
				<div className="discussion-comment-input">
					<div className="comment-input-img">
						{userDataState?.photoUrl == "" ? (
							...
							<img loading="lazy"src={userDataState?.photoUrl} width="40px" />
						)}
					</div>
					<div className="discussion-input">
						<input
							placeholder={Enter_your_comment}
							value={addDiscussionParamsState?.discussion_AR}
                            maxLength={1500}
							onKeyPress={(e)=>{
                                if (e?.key === "Enter") {
						...
						><img loading="lazy"src={DiscussionInput.src} /></span>
					</div>
				</div>
				<div className="discussion-comment-cards">
					{courseDiscussionComments?.dashboardInsideDiscussionViewModels.slice(0, DiscussionCommentCount).map(
						(item, index) => (
							<div className="discussion-comment-card">
								...
								<div className="discussion-card-text">
									<h4>{item?.userName_AR}</h4>
									<p>{item?.discussion_AR}</p>
									<div className="discussion-date">
										{/* {moment.utc(item?.insertDate, "YYYYMMDD").fromNow()} */}
										{moment.utc(item?.insertDate).fromNow()}
										{/* {moment.locale('sa')}
										{
					...
					{(courseDiscussionComments?.totalRecord === 0 || courseDiscussionComments?.totalRecord === undefined) ? (
						<p> {No_Discussion_Found} </p>
					) : (
						<>
							...
							{(DiscussionCommentCount < courseDiscussionComments?.totalRecord && courseDiscussionComments?.totalRecord !== 3) && (
								<SeeMoreBtn onClick={DiscussionSeeMoreFunc}> {Show_More} </SeeMoreBtn>
							)}
		...
		<CertificateTab>
			<p>{Get_R2_Certificate_by_completing_the_entire_course}</p>

			{(courseDetailState?.category === "Asynchronous") && (<>
				{(courseDetailState?.certificateIssue && courseDetailState?.certificateRequest) ? (<>
					{(CertificateRequestLoading) ? (
						<a disabled>{Download_Certificate} <Image alt={""} src={Rolling} width="16px" height="16px" /></a>
					) : (
						<a onClick={(e) => { e.preventDefault(); GetCertificateIssueByIdFunc() }} >{Download_Certificate}</a>
					)}
				</>) : (
					<button disabled>{Download_Certificate}</button>
				)}
			</>)}
			{(courseDetailState?.category === "Synchronous") && (<>
				{(certificateRequest) && (<>
					{(courseDetailState?.certificateRequest && courseDetailState?.certificateIssue === false) && (<>
						{(courseDetailState?.certificateStatus === "Refusal") && (
							<span>{courseDetailState?.certificateComments}</span>
						)}
						{(courseDetailState?.certificateStatus === "" || courseDetailState?.certificateStatus === null) && (
							<span>{You_have_already_applied_for_the_certificate}</span>
						)}
					</>)}
					{(courseDetailState?.certificateRequest && courseDetailState?.certificateIssue) && (<>
						{(CertificateRequestLoading) ? (
							<a disabled>{Download_Certificate} <Image alt={""} src={Rolling} width="16px" height="16px" /></a>
						) : (
							<a onClick={(e) => { e.preventDefault(); GetCertificateIssueByIdFunc() }} >{Download_Certificate}</a>
					...
					{(courseDetailState?.certificateRequest === false) && (
						<button
							onClick={(e) => {
								e.preventDefault();
								getCertificateRequestFunc()
							}}
							disabled={(CertificateRequestLoading) && ('disabled')}
						>{Request_for_Certificate} {(CertificateRequestLoading) && (<Image alt={""} src={Rolling} width="16px" height="16px" />)}</button>
					)}
				</>)}
				{/* {(!certificateRequest) && (
					// <button disabled>{Request_for_Certificate}</button>
					<button
						onClick={(e) => {
							e.preventDefault();
							getCertificateRequestFunc()
						...
						disabled={(CertificateRequestLoading) && ('disabled')}
					>{Request_for_Certificate} {(CertificateRequestLoading) && (<img loading="lazy"alt={""} src={Rolling} width="16px" height="16px" />)}</button>
				)} */}
			</>)}
		</CertificateTab>
	]
	...
	useLayoutEffect(() => {
		userReviews?.lmsUserId === userDataState?.id ? setIsRatedReview(true) : setIsRatedReview(false);
	...
	return (<>
			...
			<Col span={24} lg={15}>
				<CourseInsideDetail>
					...
					<div className="course-inside-desc">
						<h1>
							{courseDetailState?.title_AR === undefined ? (
								<SkeletonTextPlaceholder width="50%" />
							) : (
								courseDetailState?.title_AR
							)}
						</h1>
						<p>
							{Instructor}:{" "}
							{courseDetailState?.instructorName_AR === undefined ? (
								<SkeletonTextPlaceholder width="50%" />
							) : (
								courseDetailState?.instructorName_AR
							)}
						</p>
					</div>
					<CoursesTabs>
                        <div className='tabs-head'>
                            {TabsArrayKey?.map((item, index)=>(
                                <a className={(index === TabsIndex) && ('active')} onClick={()=>{clearActivetab(); setTabsIndex(index)}}> {item} </a>
                            ))}
                        </div>
								...
								<div className="curriculum-modules" key={index}>
									<div className="curriculum-module">
										<div className="curriculum-module-head">
											...
											>
												{item?.title_AR!==""?item?.title_AR:item?.title_EN}
											</h4>
											<a
												className="droparrow"
												onClick={(e) => {
											...
											<div className="module-total">
												{(item?.dashboardInsideFileViewModels.length !== 0) && (<>
													<span>
														<img loading="lazy"src={SaveImg} width="14px" />
														{(item?.dashboardInsideFileZoomViewModels.length !== 0) ? (<>
															{item?.dashboardInsideFileViewModels.length + 1} Files
														</>) : (
															item?.dashboardInsideFileViewModels.length
														)}
													</span>
												</>)}
												{(item?.dashboardInsideFileViewModels.length !== 0) && (<>
													<span>
														...
														{getMinute(item?.dashboardInsideFileViewModels)}
													</span>
												</>)}
											</div>
										</div>
											...
											{(item?.dashboardInsideFileZoomViewModels.length > 0) && (<>
												...
												{item?.dashboardInsideFileZoomViewModels?.map((item, index) => (<>
													{(item?.venueType === "Online") && (<>
														{(ZoomActive(item?.zoomDate_AR, item?.duration_AR)) ? (<>
															<div className={`curriculum-module-lesson zoom-link ${item?.viewed ? "active" : ""}`} key={index}
																onClick={() => {
																	MarkAttendanceLMSUserFunc(item?.courseTrainingLinkVenueId)
																	window.open(item?.zoomLink_AR, "_blank");
																}}
															>
																<div>
																	<h4>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</h4>
																	<span className="lesson-point"></span> {Join_a_Zoom_Meeting}
																</div>
																<ContentCenter>
															...
															{(chectZoomTimeOut(item?.zoomDate_AR, item?.duration_AR)) ? (
																<div className={`curriculum-module-lesson zoom-link disabled ${item?.viewed ? "active" : ""}`} key={index}>
																	<div>
																		<h4>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</h4>
																		<span className="lesson-point"></span> <span style={{ fontSize: "10px", fontWeight: "700", letterSpacing: "0.1px" }}>({moment.utc(item?.zoomDate_AR).local().format("MMMM DD, YYYY h:mm a")})</span>
																	</div>
																	<ContentCenter>
																		<div class="pulse">
																			<img loading="lazy"src={zoomLink?.src} width="22px" />
																...
																<div className={`curriculum-module-lesson zoom-link ${item?.viewed ? "active" : ""}`} key={index}
																	onClick={() => {
																		...
																		setStartZoomTime(item?.zoomDate_AR)
																	}}
																>
																	<div>
																		<h4>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</h4>
													...
													{(item?.venueType === "Onsite") && (
														<div className={`curriculum-module-lesson zoom-link ${item?.viewed ? "active" : ""}`} key={index}>
															...
															<ContentCenter>
																<div class="pulse">
																	<img loading="lazy"src={Onsite?.src} width="22px" />
																</div>
															</ContentCenter>
														</div>
													)}
												...
												(item, index) => (
													<>
														{item?.fileTypeRecord === "File" && (
															<div className={`curriculum-module-lesson download ${item?.viewed ? "active" : ""}`} key={index}>
																<h4>
                                                                    {(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) ? (
                                                                        <span
                                                                            onClick={() => {
                                                                                window.open(item?.url_AR!==""?item?.url_AR:item?.url_EN, "_blank")
                                                                                viewCourseFile(courseDetailState?.courseTrainingRegistrationId, item?.id, "File");
                                                                            }}>{item?.title_AR!==""?item?.title_AR:item?.title_EN}
                                                                        </span>
                                                                    ) : (
                                                                        <span
                                                                            onClick={() => {
                                                                                GetStream(courseDetailState?.courseTrainingRegistrationId, item?.id)
                                                                                viewCourseFile(courseDetailState?.courseTrainingRegistrationId, item?.id, "File");
                                                                            }}>{item?.title_AR!==""?item?.title_AR:item?.title_EN}
                                                                        </span>
                                                                    )}
                                                                    ...
                                                                    {(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) && (
                                                                        <a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank">{Download}</a>
                                                                    )}
                                                                </h4>
																<span className="dropmenu-parent">
																	<div><span className="lesson-point"></span> {item?.duration_AR} {min}</div>
																	{(item?.dashboardInsideResourceViewModels.length != 0) && (
																		<div className="resourses-drop">
																			<span onClick={(e) => {
																				e.target.parentElement.classList.toggle("active")
																			}}>{Resources} <DownOutlined style={{ fontSize: "10px" }} /></span>
																			<div className="resourses-dropmenu">
																				{item?.dashboardInsideResourceViewModels?.map((item, index) => (<>
																					{(item?.resourceTypeRecord === "File") && (
																						(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) && (
																							<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank"><DownloadOutlined /> {item?.title_AR!==""?item?.title_AR:item?.title_EN}</a>
																						)
																					)}
																					{(item?.resourceTypeRecord === "ExternalLink") && (
																						<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank"><LinkOutlined /> {item?.title_AR!==""?item?.title_AR:item?.title_EN}</a>
																					)}
																					{(item?.resourceTypeRecord === "Scorm") && (
																						<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank"><LinkOutlined /> {item?.title_AR!==""?item?.title_AR:item?.title_EN}</a>
																					)}
																				</>))}
																			</div>
														...
														{item?.fileTypeRecord === "Assessment" && (
															<div className={`curriculum-module-lesson download ${item?.viewed ? "active" : ""}`} key={index}>
																<h4>
																	<span onClick={() => {
																		router.push(`/ar/${SlugByCategory(Category)}-assessments/${item?.id}`);
																	}}
																	>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</span>
																	{(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) && (
																		<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank">Download</a>
																	)}
																</h4>
														...
														{item?.fileTypeRecord === "Survey" && (<>
															{(item?.viewed) ? (
																<div className={`curriculum-module-lesson download active disabled`} key={index}>
																	<h4>
																		<span>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</span>
																		{(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) && (
																			<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank">Download</a>
																		)}
																	</h4>
																...
																<div className={`curriculum-module-lesson download`} key={index}>
																	<h4>
																		<span
																			onClick={() => {
																				router.push(
																					`/ar/${SlugByCategory(Category)}-surveys/${item?.id}`
																				);
																			}}>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</span>
																		{(getLinkExt(item?.url_AR!==""?item?.url_AR:item?.url_EN)) && (
																			<a href={item?.url_AR!==""?item?.url_AR:item?.url_EN} target="_blank">Download</a>
														...
														{item?.fileTypeRecord === "Scorm" && (
															<div
																className={`curriculum-module-lesson ${item?.viewed ? "active" : ""
																	}`}
																key={index}
																onClick={(e) => {
																	e.preventDefault();
																	viewCourseFile(
																		item?.url_AR!==""?item?.url_AR:item?.url_EN,
																		courseDetailState?.courseTrainingRegistrationId,
																		item?.id,
																		"File"
																	);
																	window.open(
																		`${item?.url_AR!==""?item?.url_AR:item?.url_EN}?userID=112127&HASH=JHGG`
																	);
																}}
															>
																<h4>{item?.title_AR!==""?item?.title_AR:item?.title_EN}</h4>
			...
			<WriteReview dir="rtl">
				...
				{isRated && (
					<div className={(isRatingEmpty) && ("rating-empty")}>
						<StyledTextarea
							value={addReviewParamsState?.review_AR}
							onChange={(e) => {
								setIsRatingEmpty(false)
								setAddReviewParamsState({
									...addReviewParamsState,
									review_AR: e.target.value,
		...
		>
			<WriteReview dir="rtl">
				...
				{isRated && (
					<div className={(isRatingEmpty) && ("rating-empty")}>
						<StyledTextarea
							value={editReviewParamsState?.review_AR}
							onChange={(e) => {
								setIsRatingEmpty(false)
								setEditReviewParamsState({
									...editReviewParamsState,
		...
		>
			<div className="time-out-modal">
				...
				/>
				<p>{Your_zoom_class_will_start_at} <br /> <span style={{ fontWeight: "700" }}>{moment.utc(StartZoomTime).local().format("MMMM DD, YYYY h:mm:ss a")}</span></p>
				<CustomButton
					onClick={() => {
						closeZoomClassModal();
					}}
