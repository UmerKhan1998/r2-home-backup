...
import { Col, Row, Input, Popover, Divider, Skeleton, Drawer, Empty } from "antd";
import image from "next/image";
...
const SearchHeader = () => {
  ...
  const [getCourseTrainingSearchLovState, setGetCourseTrainingSearchLovState] =
    useState([]);
  const [
    getCourseTrainingSearchLovStatus,
    setGetCourseTrainingSearchLovStatus,
  ] = useState("");
  ...
  const getCourseDetailRecordFunc = async (language, search) => {
    ...
    try {
      ...
      if (response) {
        ...
        setGetCourseTrainingSearchLovStatus(response?.data?.data);
        setLoading(false);
      }
    } catch (err) {
      //console.log("err", err);
  ...
  useEffect(() => {
    // alert("Dg");
    // getCourseDetailRecordFunc("English", SearchValue);
  }, []);
  ...
  const searchOnChangeHandle = (e) => {
    ...
    setSearchValue(val);
    getCourseDetailRecordFunc("English", val);
    //console.log(val);
  };
  ...
  const SearchListItem = ({ bulletImg, item }) => (
    <SearchListItemDiv
      onClick={() => router.push(`/course-detail/${item?.id}`)}
    >
  ...
  const SearchListItemProgram = ({ bulletImg, item }) => (
    <SearchListItemDiv
      onClick={() => router.push(`/program-detail/${item?.id}`)}
    >
      <img loading="lazy"alt={""} width={20} height={20} src={bulletImg} />
      <p>{item?.title_EN}</p>
    </SearchListItemDiv>
  ...
  const SearchListItemTraining = ({ bulletImg, item }) => (
    <SearchListItemDiv
      onClick={() => router.push(`/training-detail/${item?.id}`)}
    >
      <img loading="lazy"alt={""} width={20} height={20} src={bulletImg} />
      <p>{item?.title_EN}</p>
    </SearchListItemDiv>
  ...
  const seaarchOverlay = (
    <>
      <StyledDivForSearchHeader>
        ...
        <Row>
          <Col span={24}>
            <Search
              ...
              onSearch={() => getCourseDetailRecordFunc("English", SearchValue)}
              // onSearch={searchOnSearchHandle}
            />
          </Col>
        </Row>
        <Divider type="vertical" />
        {getCourseTrainingSearchLovStatus?.length>0 ? (
          <>
            {loading ? (
              <Row
                gutter={[16, 16]}
        ...
        <SearchHeaderFooter>
          <button onClick={() => router.push(`/courses-landing-page`)}>
            View all results
          </button>
        </SearchHeaderFooter>
      </StyledDivForSearchHeader>
  ...
  const DesktopView = (
    <StyledDiv>
      <Popover
        ...
        arrowPointAtCenter
        onClick={() => getCourseDetailRecordFunc("English", SearchValue)}
      >
        <PopOverDiv>
