...
import LatestNewsCard from "../../../rtl/Cards/LatestNewsCard";
    ...
    >
      {FeaturedLatestNewsCarousalData?.map((items, index) => (
        ...
        key={index}
          id={items?.id}
          ThumbnailImage={
            items?.thumbnailImage_AR
              ? items?.thumbnailImage_AR
              : items?.thumbnailImage_EN
          }
          FeaturedType={items?.departmentName}
          CourseName={items?.title_AR ? items?.title_AR : items?.title_EN}
          Description={items?.body_AR ? items?.body_AR : items?.body_EN}
          name={name}
        />
      ))}
    </StyledCarousel>
