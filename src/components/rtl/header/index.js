...
import SearchHeader from "../../../components/rtl/Search/SearchHeader";
import endpoints from "../../../../src/api";

const { SubMenu, Item } = Menu;

const Header = ({ dropdownCaretState, setDropdownCaretState,
  setCheckFilter,
  ...
  const OurServicesArr = [
    ...
    {
      title: trainings,
      link: "ar/trainings-landing-page",
      sub_list: ["التخدير", "الجلدية", "طب القلب", "أمراض النساء والولادة"],
    },
    {
      title: programs,
      link: "ar/programs-landing-page",
      sub_list: [
        "طب الأسنان التحفظي",
        "طب الأسنان الترميمي",
        "طب أسنان عام متقدم",
    ...
    {
      title: webinars,
      link: "ar/webinars-landing-page",
      sub_list: ["التخدير", "الجلدية"],
    },
    {
      title: workshops,
      link: "ar/workshops-landing-page",
      sub_list: ["طب القلب", "أمراض النساء والولادة"],
    },

    {
      title: conferences,
      link: "ar/conferences-landing-page",
      sub_list: [
        "طب اسنان الاطفال",
        "طب الأسنان التحفظي",
        "طب الأسنان الترميمي",
    ...
    {
      title: symposiums,
      link: "ar/symposiums-landing-page",
      sub_list: [
        "طب أسنان عام متقدم",
        "طب الفم",
        "آلام الفم والوجه",
  ...
  const selectItemObject = OurServicesArr?.find(
    (item) => item?.title === serviceItemSelect
  );
  ...
  const menu = (OurServicesArr) => (
    <StyledFlexMenuDiv dir="rtl">
          ...
          <>
            {selectItemState === index ? (
              ...
              >
                <p>{item?.name_AR}</p>
                {OurServicesArr?.find(
                  (item) => item?.name_AR === serviceItemSelect
                )?.sub_list?.length > 0 && <LeftOutlined />}
              </OurServicesDesignDiv>
            ) : (
              <>
                <OurServicesDesignDiv
                  ...
                  onClick={() => router.push(`/${item?.link_AR}`)}
                >
                  <p>{item?.name_AR}</p>
                </OurServicesDesignDiv>
              </>
      ...
      <ContentMenuListChildrenDiv>
        {OurServicesArr?.find((item) => item?.name_AR === serviceItemSelect)
          ?.sub_list?.length > 0 && (
          <MenuTitleDiv>
            {
              OurServicesArr?.find(
                ...
                <ItemDiv
                  onClick={() => {
                    if(item?.recordType==="Course"){
                      router.push(`/ar/course-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Training"){
                      router.push(`/ar/training-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Program"){
                      router.push(`/ar/program-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Webinar"){
                      router.push(`/ar/webinar-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Workshop"){
                      router.push(`/ar/workshop-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Conference"){
                      router.push(`/ar/conference-detail/${item?.id}`)
                    }
                    if(item?.recordType==="Symposiums"){
                      router.push(`/ar/symposium-detail/${item?.id}`)
                    }
                    if(item?.recordType==="scholarship"){
                      router.push(`/ar/scholarship-personal-information`)
                    }
                    if(item?.recordType==="scholarshipId"){
                      router.push(`/ar/scholarship-track-application`)
                    }
                    if(serviceItemSelect===OtherServices){
                      router.push(`/ar/service-request`)
                      setCookies('other_services_item_id',item?.id?.trim());
                      setCookies('other_services_item_title', item?.title_AR?.trim());
                      dispatch(menuService({serviceId:item?.id?.trim(), serviceName:item?.title_AR?.trim()}))
                      setModalState&&setModalState(true);
                    }
                    if(serviceItemSelect===LearningInstituteText){
                      router.push(`/ar/service-request`);
                      setCookies('other_services_item_id',item?.id?.trim());
                    ...
                    if(serviceItemSelect===simulationText){
                      router.push(`/ar/service-request`);
                      setCookies('other_services_item_id',item?.id?.trim());
                      setCookies('other_services_item_title', item?.title_EN?.trim());
                      dispatch(menuService({departmentId:process.env.NEXT_PUBLIC_SIMULATION_DEPARTMENT_ID, departmentName:process.env.NEXT_PUBLIC_SIMULATION_DEPARTMENT_NAME_AR, serviceId:item?.id?.trim(), serviceName:item?.title_AR?.trim()}))
                      setModalState&&setModalState(true);
                    }                  
                  }}
                  >
        ...
        {OurServicesArr?.find((item) => item?.name_AR === serviceItemSelect)
          ?.sub_list?.length > 0 && (
            ...
            {serviceItemSelectLink!=="/ar/service-request"&&
          <CustomButton
            customStyle={{
              height: 30,
              marginTop: 15,
  ...
  const countryArr = [
    ...
    // {
    //   img: "https://i.pinimg.com/736x/28/1c/03/281c03756fadbc38d14236184c5ff7b6--flag-of-saudi-arabia-international-flags.jpg",
    //   name: arabic,
    //   link: "/ar",
    // },
  ];
    ...
    <ProfileDropdown>
      <div className="profile-dropdown">
        <div className="profile-dropdown-sub">
          <div className="profile-dropdown-name" dir="rtl">
            ...
            <div>
              <h2>
                {userDataState?.firstName_AR} {userDataState?.lastName_AR}
              </h2>
        ...
        <div className="profile-dropdown-links">
          ...
          <Link href="/ar/manage-profile">{my_profile}</Link>
          <Link href="/ar/manage-profile">{My_Account_Setting}</Link>
          <a
            onClick={() => {
              Signout();
            }}
      ...
      our_services: [
        ...
        },
        {
          ...
          sub_list: [
            ...
            title_AR:apply_for_scholarship
          },
            {
            recordType:"scholarshipId",
            title_EN:"Track your Application",
            title_AR:tracking_your_application
          },
        ],
        },
        // {
    ...
    >
      <NavDiv>
        ...
        <Container>
          ...
          {/* {router.asPath === "/" ||
          router.asPath.slice(0, 13) === "/course-detai" ||
          router.asPath.slice(0, 8) === "/about-us" ||
          !authToken ? ( */}
          <RightNav>
            {isAuthorized ? (
              <>
              ...
              <>
                ...
                <CustomButton
                  ...
                  onClick={() => Router.push("/ar/sign-in")}
                >
                  {sign_in}
                </CustomButton>
              </>
            ...
            <StyledItemMap title={tracking_your_application}>
              <AlignDiv onClick={() => router.push("/ar/track-application")}>
                <img loading="lazy"alt={""} width={24} height={24} src={MapIcon} />
              </AlignDiv>
            </StyledItemMap>
            {isAuthorized && (
              <StyledItemWishList title={Wishlist}>
                <AlignDiv onClick={() => router.push("/ar/wishlist")}>
            ...
            {/* <StyledDropdown
              onClick={() => {
                if (!dropdownCaretState) {
                  setDropdownCaretState(true);
                } else {
                  setDropdownCaretState(false);
                }
              }}
              overlay={menu}
              trigger={["click"]}
            >
              ...
              >
                <Space className={"dropdownBtnA"}>Our Services</Space>
              </a>
            </StyledDropdown>
            {!dropdownCaretState ? (
              <StyledDownOutlined />
              ...
              >
                <LanguageButtonOurServices>
                  <Space>
                    ...
                    <p style={{ color: "#000" }}>{our_services}</p>
                  </Space>
                </LanguageButtonOurServices>
              </Dropdown>
            </StyledItemOurServices>
      
            {router?.asPath === "/ar/about-us" ? (
              <StyledItemAboutUs>
                <ActiveAlignAboutUsDiv>
              ...
              {menuArr?.map((item, index) => (
                <div key={index}>
                  {item?.our_services !== undefined ? (
                      ...
                      <Dropdown
                        ...
                        <LanguageButton1>
                          <Space>
                            ...
                            <p style={{ color: "#000" }}>{item?.name_AR}</p>
                          </Space>
                        </LanguageButton1>
                      </Dropdown>
                    </StyledItemOurServices>
                    ...
                    <>
                      {router?.asPath === item?.link_AR ? (
                        <>
                          <StyledItem1>
                            <ActiveAlignDiv>
                              <Link href={`${item?.link_AR}`}>
                                {item?.name_AR}
                              </Link>
                            </ActiveAlignDiv>
                          </StyledItem1>
  ...
  const MobileView = (
    <StyledAffix offsetTop={0}>
      <MobileStyledMenu mode="horizontal" theme={theme1} dir="rtl">
        <NavDiv1>
          <MobileLogo>
            ...
            <StyledItemDropDown>
              ...
              >
                <span style={{ fontSize: "12px", color: "#000" }}>
                  AR <DownOutlined style={{ color: "#000" }} />
                </span>
              </Dropdown>
            </StyledItemDropDown>
            <StyledItemMap>
        ...
        <StyledMobSidebar className={visibleSidebarDrawer ? "show" : ""}>
          <SidebarHeader>
            ...
            <MobileRightNavToggle onClick={closeSidebarDrawer}>
              <svg
                width="30"
                height="29"
                viewBox="0 0 30 29"
          ...
          <SidebarBody>
            <ul>
                  ...
                  {item?.our_services !== undefined ? (
                    ...
                    {/* COURSES */}
                    {
                      item?.our_services[0]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[0]?.link}`}>
                            {item?.our_services[0]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[0]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[0]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[0]?.sub_list?.length > 0 ? (
                                ...
                                {item?.our_services[0]?.sub_list?.map((item) => (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      if (serviceItemSelectLink === OtherServices) {
                                        router.push(`/ar/service-request`);
                                        setCookies("other_services_item_id", item?.id?.trim());
                                        setCookies(
                                        ...
                                        dispatch(
                                          menuService({
                                            ...
                                            serviceName: item?.title_AR?.trim(),
                                          })
                                        );
                                        setModalState && setModalState(true);
                                      }
                                      if (serviceItemSelectLink === LearningInstituteText) {
                                        router.push(`/ar/service-request`);
                                        setCookies("other_services_item_id", item?.id?.trim());
                                        setCookies(
                                          "other_services_item_title",
                                      ...
                                      if (serviceItemSelectLink === simulationText) {
                                        router.push(`/ar/service-request`);
                                        setCookies("other_services_item_id", item?.id?.trim());
                                        setCookies(
                                          "other_services_item_title",
                    ...
                    {/* TRAININGS */}
                    {
                      item?.our_services[1]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[1]?.link}`}>
                            {item?.our_services[1]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[1]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[1]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[1]?.sub_list?.length > 0 ? (
                    ...
                    {/* PROGRAMS */}
                    {
                      item?.our_services[2]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[2]?.link}`}>
                            {item?.our_services[2]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[2]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[2]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[2]?.sub_list?.length > 0 ? (
                    ...
                    {/* WEBINARS */}
                    {
                      item?.our_services[3]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[3]?.link}`}>
                            {item?.our_services[3]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[3]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[3]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[3]?.sub_list?.length > 0 ? (
                    ...
                    {/* WORKSHOPS */}
                    {
                      item?.our_services[4]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[4]?.link}`}>
                            {item?.our_services[4]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[4]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[4]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[4]?.sub_list?.length > 0 ? (
                    ...
                    {/* CONFERENCES */}
                    {
                      item?.our_services[5]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[5]?.link}`}>
                            {item?.our_services[5]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[5]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[5]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[5]?.sub_list?.length > 0 ? (
                    ...
                    {/* SYMPOSIUMS */}
                    {
                      item?.our_services[6]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[6]?.link}`}>
                            {item?.our_services[6]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[6]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[6]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[6]?.sub_list?.length > 0 ? (
                    ...
                    {/* SCHOLARSHIP */}
                    {
                      item?.our_services[7]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[7]?.link}`}>
                            {item?.our_services[7]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[7]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[7]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[7]?.sub_list?.length > 0 ? (
                    ...
                    {/* SIMULATION */}
                    {
                      item?.our_services[8]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[8]?.link}`}>
                            {item?.our_services[8]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[8]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[8]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[8]?.sub_list?.length > 0 ? (
                    ...
                    
                    {
                      item?.our_services[9]?.sub_list?.length === 0 ? (
                        <StyledLinkDiv onClick={()=>setVisibleSidebarDrawer(false)}>
                          <Link key={index} href={`/ar/${item?.our_services[9]?.link}`}>
                            {item?.our_services[9]?.name_AR}
                          </Link>
                        </StyledLinkDiv>
                      ) : (
                        <Collapse
                          ...
                          accordion
                          onChange={() => {
                            setServiceItemSelectLink(item?.our_services[9]?.name_AR);
                          }}
                        >
                          <Panel
                            showArrow={false}
                            header={`${item?.our_services[9]?.name_AR}`}
                            key={`${index}`}
                          >
                            {/* <p>UK</p>  */}
                            {item?.our_services[9]?.sub_list?.length > 0 ? (
                    ...
                    <>
                    <Link key={index} href={`${item?.link_AR}`}>
                      {item?.name_AR}
                    </Link>
                    </>
                  )}
              ...
              {isAuthorized && <Link href="/ar/wishlist">قائمة الرغبات</Link>}
              <Link href="/ar/contact">يدعم</Link>

              <hr />

              {isAuthorized ? (
                <>
                  <Link href="/ar/dashboard">لوحة القيادة</Link>
                  <a
                    onClick={() => {
                      Signout();
                    }}
                ...
                <>
                  <Link href="/ar/sign-in">{sign_in}</Link>
                  <Link href="/ar/register">{register}</Link>
                </>
              )}
            </ul>
...
`;
const LanguageButtonOurServices = styled(Button)`
  height: 36px !important;
  top: -1px !important;
