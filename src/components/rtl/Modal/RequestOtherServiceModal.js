...
import { Add_Supporting_Document,  click_to_upload,email, Contact, email_is_mandatory, invalid_email, is_mandatory, Name, Number, query_detail, request_service, selected, services, Submit, your_name, your_email, enter_your_message, phone_number, service, Supporting_Document, select, department, File_Size_must_be_less_than_5MB, file_type_should_be_format } from "../../../helpers/LanguageConstant";
...
const Modal = ({
  ...
  selectService,
  contactUsFunc,
  getAllRequestaServiceLovState,
  loadingState,
  setEmailState,
  setDocumentState,
  documentState,
  getDepartmentLovState,
  setDepartFlag,
          ...
          <ModalDiv>
            ...
            <ConstenDiv>
              ...
              setSupportState({
                ...
                <p>
                يرجى ملء النموذج أدناه ، وسيقوم فريقنا بالرد وفقًا للخدمة التي اخترتها
                </p>
                    ...
                    {nameState && (
                      <>
                        {(supportState?.your_name?.trim() === "undefined undefined"||supportState?.your_name?.trim() === "") && (
                          <StyledErrorP>{Name} {is_mandatory}</StyledErrorP>
                        )}
                      </>
                    )}
                  </Col>
...
<>
                  ...
                  <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                    ...
                    <StyledSelect
                      // value="Select"
                      value={supportState?.select_dept?supportState?.select_dept:select}
                      ...
                      onChange={(value) =>{
                        ...
                        setSupportState({
                          ...supportState,
                          select_dept: value,
                          service_deptId: getDepartmentLovState?.find(item=>item?.name_EN===value)?.departmentId,
                          select_service: "",
                          service_id: "",
                        })
                      ...
                      // options={getAllRequestaServiceLovState}
                    >
                        {getDepartmentLovState?.map(
                                                (item, index) => (
                                                  <Option
                                                    key={index}
                                                    value={item?.name_EN}
...
{getAllRequestaServiceLovState?.map(
                                                (item, index) => (
                                                  <Option
                                                    key={index}
                                                    value={item?.id}
...
{selectDepartmentState&&
                  ...
                  <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                    <StyledFlexLabelDiv>
                      <RequiredP>*</RequiredP>
                      <StyledLabelP>{selected} {services}</StyledLabelP>
                    </StyledFlexLabelDiv>
                    <div style={{ position:"relative" }}>
{/* {console.log(getAllRequestaServiceLovState?.find(item=>item?.title_AR===supportState?.select_service)?.id,'getAllRequestaServiceLovState?.find')} */}
                    <StyledSelect
                      value={supportState?.select_service?supportState?.select_service:select}
                      getPopupContainer={trigger => trigger.parentNode}
                      style={{
                        width: 120,
                        textAlign:"right"
                      ...
                      onChange={(value) =>
                        setSupportState({
                          ...supportState,
                          select_service: value,
                          service_id: getAllRequestaServiceLovState?.find(item=>item?.title_AR===value)?.id,
                        })
                      }
                      // options={getAllRequestaServiceLovState}
                    >
...
{selectServiceState&&
                     ...
                     {supportState?.select_service?.trim() === "" && (
                       <StyledErrorP>{service} {is_mandatory}</StyledErrorP>
                     )}
                   </>
                    }
                  </Col>
                  ...
                  <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                    <StyledFlexLabelDiv>
                      <RequiredP>*</RequiredP>
                      <StyledLabelP>{Add_Supporting_Document}</StyledLabelP>
                    </StyledFlexLabelDiv>
              ...
              //console.log("info", info?.file?.type);
                      ...
                      >
                        <Button icon={<ImAttachment />}>{click_to_upload}</Button>
                      </StyledUpload>
                    ) : (
                      <StyledImgDiv>
                     ...
                     {documentState&& (
                      <>
                        {supportState?.supporting_doc === "" && (
                          <StyledErrorP>{Supporting_Document} {is_mandatory}</StyledErrorP>
                        )}
                      </>
                    )}
                  </Col>
                    ...
                    />
                      ...
                      <>
                        {supportState?.query_detail?.trim() === "" && (
                          <StyledErrorP>{query_detail} {is_mandatory}</StyledErrorP>
                        )}
                      </>
                    )}
                  </Col>
